...
joe_pos_neg_only.printSchema()

# COMMAND ----------

dt1 = joe_pos_neg_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('joe_total'))
dt2 = joe_pos_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('joe_pos'))
dt3 = joe_neg_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('joe_neg'))
dt4 = trump_pos_neg_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('trump_total'))
dt5 = trump_pos_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('trump_pos'))
dt6 = trump_neg_only.groupBy(F.col('state'),F.col('date_only')).agg(F.count('state').alias('trump_neg'))
...
print(df_final.show())
#df_final.write.mode('overwrite').parquet("/FileStore/tables/post_analysisdatewise/")
...
df=spark.read.parquet("/FileStore/tables/post_analysisdatewise/")
print(df.count())
print(df.show(2000))
...
df_per=df_per.select(col("state").alias("State"),col("date_only").alias("date"),"Joe Pos %","Joe Neg %","Trump Pos %","Trump Neg %")
df_per.show(2000)
...
print(df_per.show(2000,truncate=False))

# COMMAND ----------

# write to pickle file
#df_per.rdd.saveAsPickleFile('/FileStore/Final/post_analysisdatewise.pkl')
#df_per.write.mode('overwrite').parquet("/FileStore/tables/post_analysisdatewise_final/")
df_per.repartition(1).write.csv(path="/FileStore/analysisdatewise_final.csv", mode="overwrite", header="true")

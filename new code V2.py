...
#     broadcastStates.append(spark.sparkContext.broadcast(ps.map(lambda x: x).collect()).value)
    ...
    geo_true_df = spark.read.parquet("/FileStore/tables/tarun_latest/geo_true/")
...
#     print(geo_true_df.select("location","new_location").distinct().show(truncate=False,n=322306))
    # Register UDF
...
def main():
    geo_true_file_path = "/FileStore/tables/tarun_latest/geo_true/"
    spark, sc = init_spark()
    ps = sc.wholeTextFiles(r"/FileStore/tables/tarun_latest/us_state_meta.json").values().map(json.loads)
    # broadcastStates.append(ps.map(lambda x: x).collect())
    broadcastStates.append(spark.sparkContext.broadcast(ps.map(lambda x: x).collect()).value)
    ...
    #geo_true_df.show(10)
    #geo_true_df.show(truncate = False)
    
    # Explode the Array
    #geo_true_df.withColumn("new_location_2", explode(geo_true_df["new_location_1"])).show()
    #final_df = geo_true_df.withColumn("new_location_2",explode(geo_true_df["new_location_1"]))
    final_df = geo_true_df.withColumn("new_location_2",explode(geo_true_df["new_location_1"]))
    final_df.printSchema()
    df=final_df.withColumn("new_location2",col("new_location_1").cast("string"))
    #df.show()
    #df.coalesce(1).write.format('com.databricks.spark.csv').save("/path/to/file/newcodes.csv",header = 'true')
    df.printSchema()
    df = df.drop("col_name")
...
def get_new_us_code(str):
    ...
    for i in broadcastStates[0]:
        ...
        for i, v in my_dict.items():
          res[v] = [i] if v not in res.keys() else res[v] + [i]
#         max_i=max(res.keys())
#         st=max(res.items(), key=operator.itemgetter(1))[0]
        return res
...
get_new_us_code("manhattan ny")

# COMMAND ----------

 get_new_us_code("novi mi")
...
max_value = max(F.lit(all_values))

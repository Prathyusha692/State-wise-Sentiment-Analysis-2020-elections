...
def main():
    ...
    geo_udf = udf(lambda x: get_new_us_code(x), StringType())
    geo_true_df = geo_true_df.withColumn('new_location', geo_udf('new_location'))
#     print(geo_true_df.select("location","new_location").distinct().count())
    geo_true_df.select("location","new_location").write.csv("/FileStore/tables/tarun_latest/new_location/")
...
dbfs cp dbfs:FileStore/tables/tarun_latest/new_location ./test.txt

# COMMAND ----------

dbutils.fs.cp("/FileStore/tables/tarun_latest/new_location","/mnt/test/",True)
